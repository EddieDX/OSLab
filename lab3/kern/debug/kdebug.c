...
print_stackframe(void) {
     /* LAB1 YOUR CODE : STEP 1 */ //20120113670
      ...
      */
            //add code here 
    uint32_t ebp = read_ebp();//call read_ebp() to get the value of ebp. the type is (uint32_t);
    uint32_t eip = read_eip();//call read_eip() to get the value of eip. the type is (uint32_t);

    int i, j;
    for (i = 0; ebp != 0 && i < STACKFRAME_DEPTH; i ++) {
        cprintf("ebp:0x%08x eip:0x%08x args:", ebp, eip);//printf value of ebp, eip H
        ...
        eip = ((uint32_t *)ebp)[1];//ss:[ebp+4]
        ebp = ((uint32_t *)ebp)[0];//ss:[ebp]
    }
}
